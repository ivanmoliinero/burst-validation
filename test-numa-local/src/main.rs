...
fn main() {
    ...
    #[cfg(target_os = "linux")]
    {
        builder = builder.start_handler(move |thread_idx| unsafe {
            ...
            // 3. Set Memory Policy (MPOL_BIND) INSIDE the thread
    ...
    println!("Starting dual-node memory allocation test...");
    ...
    let allocation_job = |id: usize| {
        ...
        loop {
            if total_allocated_gb >= 55.0 {
                println!("[Task {}] Reached 55.0 GB limit! Holding memory without allocating more...", id);
