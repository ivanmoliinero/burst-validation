...
pub struct ScatterReduceStrategy;

impl BfsStrategy for ScatterReduceStrategy {
    fn execute(
        &self,
        params: &Input,
        ...
        for (trial, &source) in params.sources.iter().enumerate() {
            ...
            loop {
                ...
                // Phase 3: Root Evaluates & Broadcasts Global Frontier (USING SCATTER TRICK)
                let bcast_msg = if worker_id == ROOT_WORKER {
                    let combined_msg = global_frontier_msg.unwrap();
                    let mut scatter_msgs = Vec::with_capacity(actor.info.burst_size as usize);
                    for _ in 0..actor.info.burst_size {
                        scatter_msgs.push(combined_msg.clone());
                    }
                    actor
                        .scatter(Some(scatter_msgs), ROOT_WORKER)
                        .unwrap();
                    combined_msg
                } else {
                    actor.scatter(None, ROOT_WORKER).unwrap()
                };
                ...
                if worker_id == ROOT_WORKER {
                    if let Ok(elapsed) = iter_start.elapsed() {
                        println!(
                            "[Monitor ScatterReduce] Trial {} | Iter {} | Sync passed in {:.3}ms | Global Frontier: {} nodes",
                            trial,
                            current_level,
