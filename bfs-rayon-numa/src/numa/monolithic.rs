...
impl NumaPolicy for MonolithicNumaPolicy {
    fn apply_memory_policy(&self, graph: &Graph, distances: &mut [std::sync::atomic::AtomicUsize]) {
        ...
        unsafe {
            ...
            // Distances
            let dist_raw_ptr = distances.as_ptr() as usize;
            let dist_aligned_ptr = dist_raw_ptr & !(4096 - 1);
            let dist_alignment_offset = dist_raw_ptr - dist_aligned_ptr;
            let dist_bytes_to_move = (distances.len() * std::mem::size_of::<std::sync::atomic::AtomicUsize>()) + dist_alignment_offset;

            libc::syscall(
                libc::SYS_mbind,
                dist_aligned_ptr as *mut libc::c_void,
                dist_bytes_to_move,
                2,
                &mut nodemask,
                64,
            ...
            println!("Entire Graph and Distances physically pinned to Node 0.");
        }
    }
