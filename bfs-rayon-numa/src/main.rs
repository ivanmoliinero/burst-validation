...
pub mod numa;
use numa::NumaPolicy;
use numa::divided::DividedNumaPolicy;
use numa::monolithic::MonolithicNumaPolicy;
...
fn main() {
    ...
    // 1. NUMA Memory Policy
    let policy: Arc<dyn NumaPolicy> = if args.numa_divide {
    ...
    let create_pool = |worker_id: u32| {
        ...
        #[cfg(target_os = "linux")]
        {
            builder = builder.start_handler(move |_thread_idx| {
                policy_clone.apply_thread_policy(worker_id);
            });
        }
    ...
    let pool0 = create_pool(0);
    let pool1 = create_pool(1);
    ...
    timestamps.push(Timestamp { key: "worker_start".to_string(), value: start_load });
    timestamps.push(Timestamp { key: "graph_generated".to_string(), value: end_load });
        ...
        std::thread::scope(|s| {
            ...
            s.spawn(|| {
                ...
                loop {
                    ...
                    if let Ok(elapsed) = iter_start.elapsed() {
                        println!("[Node 1] Trial {} | Iter {} | Sync passed {:.3}ms | Local Frontier: {} | Remote Frontier: {}",
                            trial, current_level, elapsed.as_secs_f64() * 1000.0, next_local.len(), remote_len);
                    }
